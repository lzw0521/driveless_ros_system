...
CarMontionControl::CarMontionControl()
{
    ...
    struct itimerval time_pass,time_now;
    ...
    canSendThd = std::thread([&] {
        while (true)
        {
            ...
            ros::spinOnce();
            clock_t start ;
            if(pdata->actionSet.UpgradeRequest == up )
            {
                start = clock();
                do{
                    sendcmd();
                ...
                pdata->actionSet.UpgradeRequest = 0;
...
{
    pdata->actionSet.speedRequest = vel;
}
...
{
    pdata->actionSet.steeringRequest = angle;
}
...
void CarMontionControl::cmdMsgReceCallBack(const geometry_msgs::Twist::ConstPtr &msg)
{
    ...
    send_mutex.lock();
    if(msg->linear.y == 1)
        pdata->actionSet.unmannedMdSts = 2;
    else
        pdata->actionSet.unmannedMdSts = 1;

    pdata->actionSet.gearRequest = vecle >= 0 ? 1 : 2;
    ...
    pdata->actionSet.speedRequest = vecle;
    pdata->actionSet.steeringRequest = angle;
